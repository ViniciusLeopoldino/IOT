...
def init_csv(csv_file):
    if not os.path.exists(csv_file):
        with open(csv_file, mode='w', newline="", encoding='utf-8') as file:
            ...
            # Adicionamos a nova coluna 'localizacao'
...
def armazenar_moto():
    ...
    # Verificamos se todos os dados necessários estão presentes
    ...
    localizacao = data.get('localizacao') # Capturamos a nova informação
    ...
    try:
        with open(log_file, mode='a', newline="", encoding='utf-8') as file:
            writer = csv.writer(file)
            # Escrevemos a linha com a nova informação de localização
            writer.writerow([timestamp, placa, chassi, modelo, km, contrato, ocorrencia, localizacao])
